...
The old `vigil on` / `vigil off` verbs from v0.1.0-beta.1 are gone. There is essentially zero v0.1.0-beta.1 install base, and Vigil's stored design preferences include "no legacy / no back-compat code"; the rename is free. The one compatibility shim that survives is in `runHold` — a bare `vigil hold` (no feature argument) is treated as `vigil hold lid-awake`, because a v0.1 plist on disk hard-coded `[vigil, hold]` and might briefly invoke the new CLI before `LegacyMigration` runs on a manual DMG drag-and-drop upgrade.

## Status output and JSON schema

The v0.1 menu app screen-scraped `vigil status` line-by-line via prefix matching. That is fragile in v0.1 and would multiply across two features in v0.2. v0.2 adds `vigil status --json` as the canonical machine-readable surface; the human format remains the default for terminal users. The schema lives in `Sources/VigilCore/StatusReport.swift` with `schemaVersion = 1` so future versions can evolve cleanly.

`vigil doctor` now emits the JSON first (for the menu app's clipboard-copy diagnostics workflow), followed by a verbose human summary with `pmset -g` and `pmset -g assertions` for terminal use.
...
`LegacyMigration.runIfNeeded()` runs at most once per account, guarded by a **file marker** (`~/Library/Application Support/Vigil/.migrated-0.2.0`). It cannot be guarded by a UserDefaults key because the CLI and the menu app live in different defaults domains — whichever ran first would write its own domain's marker and the other would still think it had to migrate.

The migration: boot out the legacy `com.vigil.app.assertions` agent, remove the legacy plist, adopt any active v0.1 session into the new `lid-awake` slot as an indefinite session, then mark migration complete. The `state.json`, `visual-state.json`, and `visual-options.json` files are **preserved verbatim** — they hold the pmset and brightness snapshots that the new `LidAwakeController` still uses for restore, and renaming them would gain nothing.
...
- **AC unplugged mid-session.** Lid-Awake refuses to start on battery unless `--force-battery` (CLI) or a second-click confirmation (menu app) is given — preserved from v0.1. Caffeinate allows battery by default, since the user is present and consciously enabling display-on power.
...
- `Sources/VigilIdentifiers/VigilIdentifiers.swift` — per-feature LaunchAgent labels; legacy label preserved for migration.
- `Sources/VigilCore/` (new) — `Feature`, `Duration`, `FeatureSession`, `FeatureStateStore`, `Paths`, `StatusReport`, `LegacyMigration`.
- `Sources/vigil/` — split from one 985-line file into focused files: `main.swift` (slim dispatcher), `Shell.swift`, `State.swift`, `Power.swift` (all kept-verbatim power/display/keyboard machinery), `Privilege.swift` (kept-verbatim privileged-ops + helper-install), `LaunchAgent.swift` (per-feature plist install/bootstrap/bootout), `Features.swift` (`LidAwakeController`, `CaffeinateController`, `HoldEngine`), `Status.swift` (`StatusReport` builder + JSON/human emit).
